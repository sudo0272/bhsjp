const fetchSignUp = callback => {
    fetch('/create-account', {
        ...
        switch (text) {
            ...
            case 'no-nickname': vex.dialog.alert('닉네임이 존재하지 않습니다'); break;
            case 'nickname-length-short':
            case 'nickname-length-long': vex.dialog.alert('닉네임의 길이는 1자리에서 10자리 입니다'); break;
            case 'no-email': vex.dialog.alert('이메일이 존재하지 않습니다'); break;
            case 'email-length-short':
            case 'email-length-long': vex.dialog.alert('이메일의 길이는 @을 포함하여 3자리에서 320자리입니다'); break;
            case 'email-template-not-match': vex.dialog.alert('이메일의 형식이 맞지 않습니다'); break;
            case 'id-already-exists': vex.dialog.alert('이미 존재하는 아이디입니다'); break;
            case 'error': vex.dialog.alert('서버 에러가 발생했습니다\n다시 시도해 주세요'); break;
            case 'ok':
                vex.dialog.alert({
                    unsafeMessage: '회원가입이 완료되었습니다\n입력하신 메일로 인증메일이 발송되었으니 인증해주시고 로그인해주세요',
                    callback: () => {
